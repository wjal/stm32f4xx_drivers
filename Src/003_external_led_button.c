...
int main(void){
	...
	GPIO_button.pGPIO_x = GPIOB;
	GPIO_button.GPIO_pin_config.GPIO_pin_number = GPIO_PIN_12;
	GPIO_button.GPIO_pin_config.GPIO_pin_mode = GPIO_MODE_IN;

	GPIO_button.GPIO_pin_config.GPIO_pin_speed = GPIO_OP_SPEED_HI;
	GPIO_led.GPIO_pin_config.GPIO_pin_pdown_pup_ctrl = GPIO_NO_RESISTOR;

	GPIO_Peri_clk_ctrl(GPIOB, ENABLE);

	GPIO_init(&GPIO_button);

	uint8_t temp;
	while(1){
		temp = GPIO_read_input_pin(GPIOB, GPIO_PIN_12);
		if(!temp){
			delay();
